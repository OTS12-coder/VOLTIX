...
(function () {
  ...
  function renderFavSidebar() {
    ...
    if (!list) return;
    const items = DOTNET_PRODUCTS.filter(p => favorites.has(p.id));
    if (!items.length) {
    ...
    list.querySelectorAll('[data-fav-cart]').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        const product = DOTNET_PRODUCTS.find(p => p.id === id);
  ...
  function getFilteredProducts() {
    return DOTNET_PRODUCTS.filter(product => {
  ...
  function getCategoryName(id) {
    const cat = DOTNET_CATEGORIES.find(c => c.id === id);
    ...
    if (products.length === 0) {
      grid.innerHTML = `
        <div class="catalog__empty">
          ...
          <p>More .NET products are coming soon. Check back later or try a different category.</p>
        </div>
      `;
      return;
    }
  ...
  function initCategories() {
    ...
    const total = DOTNET_CATEGORIES.length;
    const maxVisible = 4;

    DOTNET_CATEGORIES.forEach(cat => {
  ...
  function toggleCategories() {
    ...
    updateMoreButton(moreBtn, DOTNET_CATEGORIES.length, 4);
  }
