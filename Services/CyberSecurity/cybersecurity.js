/* ==========================================================================
   VOLTIX — Cybersecurity Service Page Logic
   ========================================================================== */
...
(function () {
  ...
  const PAGE_SOURCE = 'cybersecurity';
  ...
  function renderFavSidebar() {
    ...
    if (!list) return;
    const items = CYBERSECURITY_PRODUCTS.filter(p => favorites.has(p.id));
    if (!items.length) {
    ...
    list.querySelectorAll('[data-fav-cart]').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        const product = CYBERSECURITY_PRODUCTS.find(p => p.id === id);
  ...
  function getFilteredProducts() {
    return CYBERSECURITY_PRODUCTS.filter(product => {
  ...
  function getCategoryName(id) {
    const cat = CYBERSECURITY_CATEGORIES.find(c => c.id === id);
    ...
    if (products.length === 0) {
      grid.innerHTML = `
        <div class="catalog__empty">
          ...
          <p>More cybersecurity products are coming soon. Check back later or try a different category.</p>
        </div>
      `;
      return;
    }
  ...
  function initCategories() {
    ...
    const total = CYBERSECURITY_CATEGORIES.length;
    const maxVisible = 4;

    CYBERSECURITY_CATEGORIES.forEach(cat => {
  ...
  function toggleCategories() {
    ...
    updateMoreButton(moreBtn, CYBERSECURITY_CATEGORIES.length, 4);
  }
