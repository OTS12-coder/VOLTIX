/* ==========================================================================
   VOLTIX — Frontend Service Page Logic
   ========================================================================== */
...
(function () {
  ...
  function renderFavSidebar() {
    ...
    if (!list) return;
    const items = FRONTEND_PRODUCTS.filter(p => favorites.has(p.id));
    if (!items.length) {
    ...
    list.querySelectorAll('[data-fav-cart]').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        const product = FRONTEND_PRODUCTS.find(p => p.id === id);
  ...
  function getFilteredProducts() {
    return FRONTEND_PRODUCTS.filter(product => {
  ...
  function getCategoryName(id) {
    const cat = FRONTEND_CATEGORIES.find(c => c.id === id);
    ...
    if (products.length === 0) {
      grid.innerHTML = `
        <div class="catalog__empty">
          ...
          <p>More frontend products are coming soon. Check back later or try a different category.</p>
        </div>
      `;
      return;
    }
  ...
  function initCategories() {
    ...
    const total = FRONTEND_CATEGORIES.length;
    const maxVisible = 4;

    FRONTEND_CATEGORIES.forEach(cat => {
  ...
  function toggleCategories() {
    ...
    updateMoreButton(moreBtn, FRONTEND_CATEGORIES.length, 4);
  }
