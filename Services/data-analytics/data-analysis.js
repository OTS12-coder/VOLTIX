/* ==========================================================================
   VOLTIX — Data Analytics Service Page Logic
   ========================================================================== */
...
(function () {
  ...
  const PAGE_SOURCE = 'data-analytics';
  ...
  function renderFavSidebar() {
    ...
    if (!list) return;
    const items = DATA_ANALYTICS_PRODUCTS.filter(p => favorites.has(p.id));
    if (!items.length) {
    ...
    list.querySelectorAll('[data-fav-cart]').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        const product = DATA_ANALYTICS_PRODUCTS.find(p => p.id === id);
  ...
  function getFilteredProducts() {
    return DATA_ANALYTICS_PRODUCTS.filter(product => {
  ...
  function getCategoryName(id) {
    const cat = DATA_ANALYTICS_CATEGORIES.find(c => c.id === id);
    ...
    if (products.length === 0) {
      grid.innerHTML = `
        <div class="catalog__empty">
          ...
          <p>More data analytics products are coming soon. Check back later or try a different category.</p>
        </div>
      `;
      return;
    }
